  ...
  /**
    ...
    */
    ...
    override protected def validate() {
      ...
      if (!getConnectors.contains(Connector.Type.COMPUTER)) throw new IllegalStateException("Validation failed: hub has no computer connector.")
      // Check if the hub has no peripheral connector
      if (!getConnectors.contains(Connector.Type.PERIPHERAL)) throw new IllegalStateException("Validation failed: hub has no peripheral connector.")
    }
  }
